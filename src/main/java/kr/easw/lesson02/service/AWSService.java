...
public class AWSService {
    ...
    public void upload(MultipartFile file) {
        ...
        // 허용되는 파일 확장자 목록
        List<String> allowedExtensions = Arrays.asList("pdf", "zip", "jpg", "jpeg", "png", "gif", "csv", "exe", "hwp", "pptx", "doc", "docx", "mp4", "avi", "xlsx");

        if (!allowedExtensions.contains(fileExtension)) {
            throw new IllegalArgumentException("허용되지 않는 파일 확장자입니다.");
        }

        // 허용된 파일 확장자의 경우에만 업로드 진행
        s3Client.putObject(BUCKET_NAME, originalFilename, file.getInputStream(), new ObjectMetadata());
    }
